...
import { BrainCircuit, ArrowLeft, Zap, Dna, Plus, Download, AlertCircle, RefreshCw, Send, MessageSquare, Users } from "lucide-react";
...
export default function ComparePage() {
  ...
  const handleAiAnalysis = async () => {
    ...
    try {
      ...
      setAnalysisError("Analysis failed. Please check your API key and try again.");
    } finally {
    ...
    <div className="min-h-screen bg-background p-6 md:p-12 animate-in fade-in duration-700">
      <div className="max-w-7xl mx-auto space-y-12">
        ...
        <div className="space-y-4">
          <h1 className="text-4xl md:text-5xl font-serif font-medium tracking-tight">Relationship <span className="text-primary italic">Dynamics</span></h1>
          <p className="text-lg text-muted-foreground">Select multiple profiles to analyze compatibility and friction clusters.</p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
          {profiles.map(p => (
            <button
              ...
              className={`p-3 rounded-2xl border transition-all text-left relative overflow-hidden ${selectedIds.includes(p.id) ? 'border-primary bg-primary/5 ring-1 ring-primary' : 'border-border bg-card hover:border-primary/40'}`}
            >
          ...
          <Link href="/test">
            <button className="p-3 rounded-2xl border border-dashed border-border hover:border-primary/50 flex flex-col items-center justify-center gap-1 group h-full">
        ...
        {selectedProfiles.length >= 2 ? (
          <div className="grid xl:grid-cols-12 gap-10">
            ...
            <div className="xl:col-span-7 space-y-10">
              <div className="bg-card border border-border/40 p-8 rounded-[40px] shadow-sm">
                <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-6">Divergence Heatmap</h3>
              ...
              {selectedProfiles.length === 2 ? (
                <div className="grid md:grid-cols-2 gap-8">
                  <div className="bg-card border border-border/40 p-6 rounded-[32px]">
                    <h3 className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-4">Structural Compare</h3>
                    <RadarChart 
                  ...
                  <div className="bg-card border border-border/40 p-6 rounded-[32px]">
                    <h3 className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground mb-4">Linear Map</h3>
                    <ResultsChart 
                      scores={selectedProfiles[0].scores}
                      scores2={selectedProfiles[1].scores}
                      label1={selectedProfiles[0].name}
                      label2={selectedProfiles[1].name}
                    />
                  </div>
                </div>
              ) : (
                <div className="bg-primary/5 p-10 rounded-[40px] border border-dashed border-primary/20 text-center">
                  <Users className="w-10 h-10 text-primary/30 mx-auto mb-4" />
                  <p className="text-muted-foreground text-sm">Select exactly two profiles for detailed linear and radar comparisons.</p>
                </div>
              )}
            </div>
            ...
            <div className="xl:col-span-5 space-y-8">
              {/* Analysis Trigger (Only for 2 people) */}
              {selectedProfiles.length === 2 && !aiAnalysis && (
                <Button onClick={handleAiAnalysis} disabled={isAnalyzing} className="w-full h-16 rounded-3xl shadow-xl shadow-primary/10">
                  {isAnalyzing ? <RefreshCw className="animate-spin mr-2" /> : <BrainCircuit className="mr-2" />}
                  Generate Deep Conflict Analysis
                </Button>
              )}

              {/* Analysis Results */}
              {aiAnalysis && (
                <div className="bg-primary text-primary-foreground p-8 rounded-[40px] shadow-2xl animate-in fade-in slide-in-from-bottom-4">
                  <h3 className="text-[10px] font-bold uppercase tracking-widest text-primary-foreground/40 mb-4">Core Friction Analysis</h3>
                  <p className="text-lg font-serif italic mb-6">"{aiAnalysis.overview}"</p>
                  <div className="space-y-3">
                    {aiAnalysis.frictionPoints.map((fp, i) => (
                      <div key={i} className="bg-white/10 p-4 rounded-xl text-sm border border-white/5">{fp}</div>
                    ))}
                  </div>
                  <Button variant="ghost" onClick={() => setAiAnalysis(null)} className="mt-6 text-white/40 hover:text-white h-8 text-[10px]">Reset Analysis</Button>
                </div>
              )}
              ...
              <div className="bg-slate-900 text-slate-100 rounded-[40px] flex flex-col h-[500px] shadow-2xl overflow-hidden border border-slate-800">
                <div className="p-5 border-b border-slate-800 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-widest">Relationship Advisor</span>
                  </div>
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-4">
                  {chatHistory.length === 0 && (
                    <div className="text-center py-10 opacity-30">
                       <p className="text-xs">Ask specific questions about these personalities.</p>
                       <p className="text-[10px] mt-2 italic">"How can Person A help Person B feel more secure?"</p>
                    </div>
                  )}
                  ...
                  {isChatting && <div className="text-[10px] text-slate-500 animate-pulse">Thinking...</div>}
                  <div ref={chatEndRef} />
                </div>
                ...
                <form onSubmit={handleChat} className="p-4 bg-slate-800/50 border-t border-slate-800 flex gap-2">
                  <input 
                    ...
                    placeholder="Ask about these dynamics..."
                    className="flex-1 bg-slate-900 border border-slate-700 rounded-full px-4 py-2 text-xs focus:ring-1 focus:ring-primary outline-none"
                  />
                  <button type="submit" className="bg-primary text-white p-2 rounded-full hover:scale-105 transition-transform">
          ...
          <div className="bg-muted/10 border border-dashed border-border rounded-[48px] p-24 text-center">
            ...
            <h2 className="text-2xl font-serif text-muted-foreground">Select at least two profiles to map dynamics.</h2>
          </div>
        )}
      </div>
    </div>

 ...
 * of the entire ComparePage during typing (fixes INP issue).
 */
const ChatAdvisor = memo(({ selectedProfiles }: { selectedProfiles: Profile[] }) => {
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-4 scroll-smooth">
        {chatHistory.length === 0 && (
          <div className="text-center py-12 opacity-30 px-6">
             ...
             <p className="text-[10px] mt-2 italic">"Who is most likely to cause tension in high stress?"</p>
          </div>
        )}
        ...
        {selectedProfiles.length >= 2 ? (
          <div className="grid xl:grid-cols-12 gap-10">
            ...
            <div className="xl:col-span-7 space-y-10">
              <div className="bg-card border border-border/40 p-8 rounded-[40px] shadow-sm">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Group Divergence Map</h3>
              ...
              {selectedProfiles.length === 2 ? (
                <div className="grid md:grid-cols-2 gap-8">
                  ...
                  <div className="bg-card border border-border/40 p-6 rounded-[32px]">
                    ...
                    <ResultsChart 
                      ...
                      profileId1={selectedProfiles[0].id}
                      profileId2={selectedProfiles[1].id}
                    />
                  </div>
                </div>
              ) : (
                <div className="bg-secondary/20 p-10 rounded-[40px] border border-dashed border-border text-center">
                  ...
                  <p className="text-muted-foreground text-sm max-w-sm mx-auto">The heatmap above represents the total divergence across all {selectedProfiles.length} members.</p>
                </div>
              )}
            </div>
            ...
            <div className="xl:col-span-5 space-y-8">
              ...
              {/* Localized ChatAdvisor prevents typing latency */}
              <ChatAdvisor selectedProfiles={selectedProfiles} />
            </div>
          </div>
        ) : (
          <div className="bg-muted/10 border border-dashed border-border rounded-[48px] p-24 text-center">
            ...
            <h2 className="text-2xl font-serif text-muted-foreground">Select two or more profiles.</h2>
            <p className="text-sm text-muted-foreground mt-2 italic max-w-sm mx-auto">Compare personalities to see clusters and friction points.</p>
          </div>
        )}
      </div>
    </div>
